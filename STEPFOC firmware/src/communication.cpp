...
void UART_protocol(Stream &Serialport)
{
    ...
    while (Serialport.available())
    {
        ...
        if (parser.parse(c, command, argument))
        {
            ...
            // Go to voltage torque mode
            // Sets/ gets Uq setpoint
            ...
            // Set Coast
            else if (strcmp(command, "Coast") == 0)
            {

                controller.Controller_mode = 0;
                controller.brake_coast = 0;
                Serialport.print("Coast ");
                Serialport.println(controller.brake_coast);
            }

            // Get/Set PID voltage limit
            ...
            else if (strcmp(command, "PWM") == 0)
            {
                ...
                Serialport.print("PWM4 ");
                Serialport.println(FOC.PWM4);
            }
            ...
            // NOTE also used for voltage of the open loop control
            ...
            // Set/get open loop stepper mode microstep
            else if (strcmp(command, "microstep") == 0)
            {

                if (strlen(argument) != 0)
                {
                    controller.microstep = atoi(argument);
                }
                Serialport.print("Microstep is: ");
                Serialport.println(controller.microstep);
            }

            // Set/get open loop stepper mode step interval
            else if (strcmp(command, "stepinterval") == 0)
            {

                if (strlen(argument) != 0)
                {
                    controller.step_interval = atoi(argument);
                }
                Serialport.print("Step interval is: ");
                Serialport.println(controller.step_interval);
            }

            // Set/get open loop stepper mode microstep
            else if (strcmp(command, "gosteps") == 0)
            {

                if (strlen(argument) != 0)
                {
                    controller.Controller_mode = 10;
                    controller.open_loop_move_done = 0;
                    controller.gosteps = atoi(argument);
                }
                Serialport.print("Moving for ");
                Serialport.print(controller.gosteps);
            ...
            else if (strcmp(command, "Info") == 0)
            {
                ...
                Serialport.println(controller.theta_offset);
            ...
            // NOTE that for open loop you can also set voltage with: Openv
