...
typedef struct
{
    ...
    volatile int pole_pairs = 50;                   // Number of pole pairs of your Stepper motor
    ...
    volatile int Controller_mode = 0; // 0 -> idle, 1 -> Positon, 2 -> Speed, 3 -> Current, 4 -> PD, 5 -> Open-loop speed, 6 -> Voltage Torque mode
    ...
    volatile bool Calibrated = 1;                    // 0 if motor is not calibrated, 1 if it is calibrated
    ...
    volatile int brake_coast = 1; // 1 is brake 0 is coast
...
typedef struct
{
    ...
    volatile int Voltage_limit = 6000; // Voltage limit for Ud and Uq voltages. If 0 value of controller.VBUS_mV will be used
    // if != 0 it will use that number (if bigger than controller.VBUS_mV it will use controller.VBUS_mV)
    // This is extremely important for proper operation of Stepper motor loops since their resistances can be less
    // than 1 ohm and using full range of 24V makes the loop unstable.
    // How to setup:
    // Phase resistance < 2; Voltage_limit = 6000;
...
typedef struct
{
    ...
    volatile uint8_t speed_setpoint = 50;
    volatile int current_setpoint = 400;
    ...
    // Map this to 255 and 0
    volatile int max_speed = 60000;
    volatile int min_speed = 500;
