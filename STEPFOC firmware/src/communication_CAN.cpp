...
/// @brief  Node id is 4 bits, CAN command is 6 bits and error bit is 1 bit
        ...
        if (Node_ID == controller.CAN_ID)
        {
            ...
            case IN_HALL_INDEX:{
                ...
                if(CAN_RX_msg.len == 4){
                    ...
                    #if (DEBUG_COMS > 0)
                    Serialport.println("Received HALL INDEX");
                    #endif
                }else{
                    #if (DEBUG_COMS > 0)
                    Serialport.println("HALL INDEX; Wrong DL");
                    #endif 
                    controller.Wrong_DL = 1;
                }
                HALL_data_pack();
...
void HALL_data_pack()
{
    ...
    CAN_TX_msg.data[1] = data_buffer_send[1];
    CAN_TX_msg.data[2] = data_buffer_send[2];
    CAN_TX_msg.data[3] = bitsToByte(HALL_array);
    CAN_TX_msg.len = 4;
    CAN_TX_msg.type = DATA_FRAME;
    CAN_TX_msg.format = STANDARD_FORMAT;
    CAN_TX_msg.id = Combine_2_CAN_ID(controller.CAN_ID, OUT_DATA_HALL, controller.Error);
    CANSend(&CAN_TX_msg);
    controller.Send_heartbeat = 0;
}
